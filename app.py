...
st.title("🎯 블록 스나이퍼: 요원 SJI")
st.markdown("**[리얼 스나이퍼 조작법]**\n* 화면을 **꾹 누르면** 조준(Zoom) 시작!\n* 손가락을 **떼면** 발사(탕!) 후 즉시 **엄폐**\n* ⚠️ 붉은 레이저 조준 시 **무조건 숨으세요!**")

# 🔍 이미지 자동 탐색 시스템 (대소문자, 확장자 모두 커버)
possible_names = [
    "sniper.jpg", "sniper.JPG", "Sniper.jpg", "SNIPER.jpg", 
    "sniper.png", "sniper.PNG", "sniper.jpeg", "sniper.JPEG"
]
...
if found_file:
    try:
        ...
        st.success(f"✅ 전술 요원 프로필 [{found_file}] 로딩 완료! 작전을 시작합니다.")
    except Exception as e:
        st.error("❌ 이미지 파일을 읽는 중 오류가 발생했습니다.")
else:
    st.error("❌ 'sniper' 이미지 파일을 찾을 수 없습니다. 깃허브에 파일이 잘 올라갔는지 확인해주세요.")
...
<script>
  ...
  // ★ 파이썬에서 넘겨준 이미지 데이터 수신
  const sniperImg = new Image();
  const base64Data = "SNIPER_IMG_SRC";
  if(base64Data.startsWith("data:")) {
      sniperImg.src = base64Data;
  }
  ...
  let isHiding = true; let currentWallY = canvas.height * 0.6; 
  ...
  // ★ 빈 액자 버그 해결! 이미지가 100% 로딩되었을 때만 그리도록 방어 코드 추가
  function drawFemaleSniper() {
      let frameW = 160; let frameH = 220;
      let frameX = canvas.width - frameW - 15; let frameY = canvas.height - frameH - 15;

      if (sniperImg.complete && sniperImg.naturalWidth > 0) {
          ctx.save();
          ctx.shadowColor = "rgba(0,0,0,0.8)"; ctx.shadowBlur = 15;
          ctx.strokeStyle = "#FFD700"; ctx.lineWidth = 3;
          ctx.strokeRect(frameX, frameY, frameW, frameH); ctx.shadowBlur = 0;
          
          ctx.drawImage(sniperImg, frameX, frameY, frameW, frameH);
          
          ctx.fillStyle = "rgba(0,0,0,0.7)"; ctx.fillRect(frameX, frameY + frameH - 30, frameW, 30);
          ctx.fillStyle = "#FFF"; ctx.font = "16px Arial"; ctx.textAlign = "center";
          ctx.fillText("AGENT SJI", frameX + frameW/2, frameY + frameH - 10);
          ctx.restore();
      } else {
          // 이미지를 못 찾았다면 찰흙 인형을 띄움 (빈 박스 방지)
          ctx.save();
          ctx.fillStyle = "#1e272e"; ctx.beginPath(); ctx.moveTo(-10, canvas.height); ctx.lineTo(30, canvas.height - 180); ctx.lineTo(130, canvas.height - 160); ctx.lineTo(160, canvas.height); ctx.fill();
  ...
  function drawMinecraftEnemy(t) {
      if(t.subType === 'zombie') { 
          drawBlock(t.x, t.y, t.size, t.size, "#2ecc71"); drawBlock(t.x + t.size*0.1, t.y + t.size, t.size*0.8, t.size*1.2, "#3498db");
      } else if (t.subType === 'skeleton') { 
          drawBlock(t.x, t.y, t.size, t.size, "#ecf0f1"); drawBlock(t.x + t.size*0.1, t.y + t.size, t.size*0.8, t.size*1.2, "#bdc3c7");
          ...
          drawBlock(t.x + t.size*0.2, t.y + t.size*0.7, t.size*0.2, t.size*0.2, "#e74c3c"); drawBlock(t.x + t.size*0.6, t.y + t.size*0.7, t.size*0.2, t.size*0.2, "#e74c3c"); 
      } else if (t.subType === 'enderman') { 
          drawBlock(t.x, t.y - t.size*0.5, t.size*0.8, t.size*0.8, "#111"); 
          drawBlock(t.x + t.size*0.1, t.y + t.size*0.3, t.size*0.6, t.size*2.5, "#111"); drawBlock(t.x + t.size*0.1, t.y - t.size*0.2, t.size*0.6, t.size*0.15, "#9b59b6"); 
      }
  }

  function spawnTarget() {
      if(targets.length < 5) { 
          let types = ['zombie', 'skeleton', 'spider', 'enderman']; let sub = types[Math.floor(Math.random() * types.length)];
          ...
          targets.push({ subType: sub, x: speedX > 0 ? -60 : canvas.width + 60, y: yPos, size: size, speedX: speedX, attackTimer: Math.random() * 80 + 100 });
      }
  }

  function shoot() {
      if(gameOver) return;
      recoilOffset = 25; ctx.fillStyle = "rgba(255, 255, 0, 0.5)"; ctx.fillRect(0, 0, canvas.width, canvas.height); 
      ...
      for (let i = targets.length - 1; i >= 0; i--) {
          ...
          if (mouseX > t.x - 20 && mouseX < t.x + t.size*2 + 20 && mouseY > t.y - 30 && mouseY < t.y + tHeight + 20) {
              if(mouseY < t.y + t.size + 10) { score += 2; createParticles(mouseX, mouseY, "#e74c3c"); } 
              else { score += 1; createParticles(mouseX, mouseY, "#3498db"); }
  ...
  function createParticles(x, y, color) { for(let i=0; i<15; i++) particles.push({ x: x, y: y, vx: (Math.random() - 0.5)*15, vy: (Math.random() - 0.5)*15, size: Math.random()*6+4, color: color, life: 20 }); }
  ...
  function drawScope() {
      ctx.save();
      let currentY = mouseY - recoilOffset; if(recoilOffset > 0) recoilOffset -= 3; 
      let scopeRadius = 140; 
      ...
      ctx.beginPath(); ctx.moveTo(mouseX - scopeRadius, currentY); ctx.lineTo(mouseX + scopeRadius, currentY); ctx.moveTo(mouseX, currentY - scopeRadius); ctx.lineTo(mouseX, currentY + scopeRadius); ctx.stroke();
  ...
  function update() {
      ...
      let targetWallY = isHiding ? canvas.height * 0.55 : canvas.height; currentWallY += (targetWallY - currentWallY) * 0.25; 

      for (let i = targets.length - 1; i >= 0; i--) {
          let t = targets[i]; t.x += t.speedX;
          if(t.subType === 'enderman' && frameCount % 60 === 0 && Math.random() < 0.5) { t.x += (Math.random() > 0.5 ? 40 : -40); createParticles(t.x, t.y, "#9b59b6"); }
          if(t.x < -80 || t.x > canvas.width + 80) targets.splice(i, 1); 
          else { t.attackTimer--; if(t.attackTimer <= 0) { enemyBullets.push({ startX: t.x + t.size/2, startY: t.y, progress: 0 }); t.attackTimer = Math.random() * 100 + 100; } }
      }
      ...
      for (let i = enemyBullets.length - 1; i >= 0; i--) {
          ...
          if(b.progress >= 1) { 
              if(!isHiding) {
                  ...
                  createParticles(canvas.width/2, canvas.height/2, "#FF0000"); if(hp <= 0) handleGameOver();
              } else { createParticles(canvas.width/2, canvas.height*0.7, "#f1c40f"); }
  ...
  function gameLoop() {
      if (gameOver) return;
      update(); ctx.clearRect(0, 0, canvas.width, canvas.height); drawBackground();
      ...
      for (let t of targets) {
          ...
          if(t.attackTimer > 0 && t.attackTimer < 60) {
              ...
              ctx.fillStyle = "rgba(255, 0, 0, 0.9)"; ctx.font = "22px Arial"; ctx.textAlign = "center"; ctx.fillText("⚠️ 적 조준 중!! 숨으세요!", canvas.width/2, 80); ctx.restore();
          }
      }
      for (let p of particles) { ctx.fillStyle = p.color; ctx.fillRect(p.x, p.y, p.size, p.size); }
      if(!isHiding) drawScope(); 
      for (let b of enemyBullets) {
          let currX = b.startX + (canvas.width/2 - b.startX) * b.progress; let currY = b.startY + (canvas.height/2 - b.startY) * b.progress; let currRadius = 5 + b.progress * 60; 
  ...
  canvas.addEventListener("mouseup", endAim); canvas.addEventListener("mouseleave", () => { if(!isHiding) endAim(); });
  ...
  function handleGameOver() {
      ...
      let isTop5 = false; if (score > 0 && (highScores.length < 5 || score > highScores[highScores.length - 1].score)) isTop5 = true;
      if (isTop5) { document.getElementById("new-record-input").style.display = "block"; document.getElementById("leaderboard").style.display = "none"; document.getElementById("initials").value = ""; 
      } else { document.getElementById("new-record-input").style.display = "none"; showLeaderboard(); }
  }

  function saveScore() {
      let initials = document.getElementById("initials").value.toUpperCase() || "UNK";
      highScores.push({name: initials.substring(0,3), score: score}); highScores.sort((a,b) => b.score - a.score); highScores = highScores.slice(0,5); 
